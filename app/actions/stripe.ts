...
export async function createCheckoutSession(data: CheckoutSessionData) {
  try {
    ...
    console.log("[YeneBakery] Total amount:", data.total);
    console.log("[YeneBakery] Items count:", data.items.length);
    ...
    console.log(
      "[YeneBakery] Creating Stripe session with",
      lineItems.length,
      "line items",
    );
    ...
    console.log(
      "[YeneBakery] Stripe session created successfully:",
      session.id,
    );
    return { clientSecret: session.client_secret };
  } catch (error) {
    console.error(
      "[YeneBakery] Error creating Stripe checkout session:",
      error,
    );
    if (error instanceof Error) {
      console.error("[YeneBakery] Error message:", error.message);
