  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.username =
          (user as { username?: string }).username ?? token.name ?? undefined;
        token.fullName =
          (user as { fullName?: string }).fullName ?? token.name ?? undefined;
    ...
    async session({ session, token }) {
      session.user = {
        name:
          (token as { fullName?: string; username?: string; name?: string })
            .fullName ??
          token.name ??
          null,
